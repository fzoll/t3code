...
const resolveCallbackTurnId = (ctx: JcodeSessionContext): TurnId | undefined => ctx.activeTurnId;
...
export function makeJcodeAdapter(jcodeSettings: JcodeSettings, options?: JcodeAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const startSession: JcodeAdapterShape["startSession"] = (input) =>
      withThreadLock(
        input.threadId,
        Effect.gen(function* () {
          ...
          const requestedStartModelId = jcodeModelSelection?.model
            ...
            : undefined;
          const boundModelId = yield* applyJcodeAcpModelSelection({
            runtime: acp,
          ...
          const session: ProviderSession = {
            ...
            cwd,
            ...(boundModelId ? { model: resolveJcodeAcpBaseModelId(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
    ...
    const sendTurn: JcodeAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const requestedTurnModelId = turnModelSelection?.model
                ...
                : undefined;
              const currentModelId = yield* applyJcodeAcpModelSelection({
                runtime: ctx.acp,
              ...
              const displayModel = currentModelId
                ? resolveJcodeAcpBaseModelId(currentModelId)
                : undefined;
    ...
    const interruptTurn: JcodeAdapterShape["interruptTurn"] = (threadId, turnId) =>
      Effect.gen(function* () {
    ...
    const readThread: JcodeAdapterShape["readThread"] = (threadId) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
        return { threadId, turns: ctx.turns };
      });

    const rollbackThread: JcodeAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        yield* requireSession(threadId);
    ...
    const stopSession: JcodeAdapterShape["stopSession"] = (threadId) =>
      withThreadLock(
        threadId,
        Effect.gen(function* () {
          const ctx = yield* requireSession(threadId);

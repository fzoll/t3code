...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const fetchRemote: GitVcsDriver.GitVcsDriver["Service"]["fetchRemote"] = Effect.fn("fetchRemote")(
    function* (input) {
      ...
      const fetchAll = withRepoGitLock(
        input.cwd,
        executeGit("GitVcsDriver.fetchRemote", input.cwd, args, options),
      );
      ...
      const result = yield* withRepoGitLock(
        input.cwd,
        executeGitWithStableDiagnostics("GitVcsDriver.fetchRemote", input.cwd, scopedArgs, {
          ...options,
          allowNonZeroExit: true,
        }),
      );

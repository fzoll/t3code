    ...
    yield* sql`
      ...
      FROM (
        SELECT
          ...
          created_at,
          ROW_NUMBER() OVER (
            PARTITION BY payload_json::jsonb->>'requestId'
            ORDER BY created_at ASC, activity_id ASC
          ) AS row_number
        FROM projection_thread_activities
        WHERE kind = 'approval.requested'
          AND payload_json::jsonb->>'requestId' IS NOT NULL
      ) AS requested
      ...
      WITH latest_resolutions AS (
        ...
        FROM (
          SELECT
            ...
            CASE
              WHEN payload_json::jsonb->>'decision' IN (
                'accept',
                'acceptForSession',
                'decline',
                'cancel'
              )
              THEN payload_json::jsonb->>'decision'
              ELSE NULL
            END AS decision,
            ROW_NUMBER() OVER (
              PARTITION BY payload_json::jsonb->>'requestId'
      ...
      WITH latest_stale_failures AS (
        ...
        FROM (
          ...
          WHERE kind = 'provider.approval.respond.failed'
            AND payload_json::jsonb->>'requestId' IS NOT NULL
            AND (
              lower(COALESCE(payload_json::jsonb->>'detail', ''))
                LIKE '%stale pending approval request%'
              OR lower(COALESCE(payload_json::jsonb->>'detail', ''))
      ...
      SET
        ...
        pending_user_input_count = COALESCE((
          WITH latest_user_input_states AS (
            ...
            FROM (
              SELECT
                activity.payload_json::jsonb->>'requestId' AS request_id,
                activity.kind,
                lower(COALESCE(activity.payload_json::jsonb->>'detail', '')) AS detail,
                ROW_NUMBER() OVER (
                  PARTITION BY activity.payload_json::jsonb->>'requestId'
                  ORDER BY activity.created_at DESC, activity.activity_id DESC
                ) AS row_number
              FROM projection_thread_activities AS activity
              WHERE activity.thread_id = projection_threads.thread_id
                AND activity.payload_json::jsonb->>'requestId' IS NOT NULL

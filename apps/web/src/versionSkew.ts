...
// Primary/local connections opt in to build-metadata comparison: the client is
// served by the very server it talks to, so a `+git.<sha>` drift between the
// stale bundle and a moved-on source-mode checkout (#1) is actionable. Remote
// and secondary peers leave this off — two healthy nodes in a fleet routinely
// sit on different commits and must not warn on build metadata alone.
 ...
 *
 * With `compareBuildMetadata`, any exact version difference — including the
 * `+git.<sha>` build suffix — is treated as drift, since the primary connection
 * expects the client and its own server to be byte-identical.
 */
export function resolveVersionMismatch(
  ...
  if (options?.compareBuildMetadata) {
    if (normalizedClientVersion === normalizedServerVersion) {
      return null;
    }
    return {
